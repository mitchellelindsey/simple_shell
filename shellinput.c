...
int mv_strcmp(const char *string1, const char *string2);
void signal_handler(int signal);
size_t mv_strlen(const char *str);
char *mv_strdup(const char *str);
char *mv_strcat(char *final_str, char *str_to);
char *mv_strcpy(char *dest, char *src);
...
int main(int ac, char **av)
{
	...
	while (1)
	{
		...
		{
			prompt_bytes_written = write(1, prompt, mv_strlen(prompt));
		...
		characters_read = mv_getline(&line, &len, stdin);
		/*printf("line is: %s\n", line);*/
		...
		if (mv_strcmp(line, "env") == 0)
		{
			...
			for (; environ[i]; i++)
			{
				...
				write(1, environ[i], mv_strlen(environ[i]));
				write(1, "\n", 1);
			}
			free(line);
			continue;
		...
		while (command_token != NULL)
		{
			...
			command[i] = mv_strdup(command_token);
			if (command[i] == NULL)
				continue;
		...
		if (path == NULL)
		{
			...
			while (directories_token != NULL)
			{
				...
				directories[directories_i] = mv_strdup(directories_token);
				if (PATH == NULL)
					continue;
			...
			while (directories[searching_i] != NULL)
			{
				...
				while (entry != NULL)
				{
					if (mv_strcmp(command[0], entry->d_name) == 0)
					{
						found = 1;
						break;
					}
				...
				if (searching_i == directories_i)/*directories[searching_i] == NULL*/
				{
					...
					/*write(2, line, mv_strlen(line));*/
					/*write(2, err_msg, mv_strlen(err_msg));*/
					/*found = -1;*/
					/* err_msg = NULL;*/
					perror(command[0]);
				}
		...
		else if (child_pid == 0)
		{
			...
			{
				size = mv_strlen(directories[searching_i]) + mv_strlen(line) + 2;
				...
				mv_strcpy(concat_path, directories[searching_i]);
				mv_strcat(concat_path, "/");
				mv_strcat(concat_path, command[0]/*line*/);
